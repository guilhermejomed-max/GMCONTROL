...
export const RetreadingHub: React.FC<RetreadingHubProps> = ({ orgId, tires, retreadOrders, onUpdateTire, onNotification, settings }) => {
  ...
  useEffect(() => {
     const unsubP = storageService.subscribeToRetreaders(orgId, setPartners);
     const unsubT = storageService.subscribeToTreadPatterns(orgId, setTreadPatterns);
  ...
  const handleSendToRetread = async () => {
    ...
    await storageService.updateTireBatch(orgId, updates);
  ...
  const handleAddPartner = async () => {
      ...
      await storageService.addRetreader(orgId, newPartnerName.trim());
      setNewPartnerName('');
  ...
  const handleReceiveTires = async () => {
    ...
    await storageService.updateRetreadOrder(orgId, receivingOrder.id, {
        status: 'CONCLUIDO',
