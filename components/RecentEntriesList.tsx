...
export const RecentEntriesList: React.FC<RecentEntriesListProps> = React.memo(({ 
  ...
  return (
    <div className="space-y-2 flex-1 overflow-y-auto pr-1 custom-scrollbar w-full h-full min-h-[260px]">
      {entries.map((entry) => {
        const entryAvg = calculateEntryEfficiency(entry, allFuelEntries);
        const volume = getFuelVolume(entry);

        return (
          <div key={entry.id} className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm group relative overflow-hidden hover:border-blue-200 dark:hover:border-blue-800 transition-all">
            ...
            <div className="flex justify-between items-end pt-1.5 border-t border-slate-50 dark:border-slate-800/50">
              <div className="space-y-0.5 min-w-0 pr-2">
                <p className="text-[9px] text-slate-500 font-medium flex items-center gap-1 truncate">
                  <MapPin className="h-2.5 w-2.5 text-slate-400 shrink-0" /> <span className="truncate">{entry.stationName || 'Posto nao inf.'}</span>
                </p>
                <p className="text-[9px] text-slate-500 font-medium flex items-center gap-1 truncate">
                  <User className="h-2.5 w-2.5 text-slate-400 shrink-0" /> <span className="truncate">{entry.driverName || 'Motorista nao inf.'}</span>
                </p>
              </div>
