...
export const FuelDashboard: React.FC<Props> = ({
  ...
  const recentVehicleEntries = useMemo(() => {
    const latestByVehicle = new Map<string, FuelEntry>();
    fuelEntries.forEach(entry => {
      const key = entry.vehicleId || entry.vehiclePlate || entry.id;
      const current = latestByVehicle.get(key);
      if (!current || sortEntriesDesc(entry, current) < 0) {
        latestByVehicle.set(key, entry);
      }
    });
    return Array.from(latestByVehicle.values()).sort(sortEntriesDesc);
  }, [fuelEntries]);

  const paginatedEntries = useMemo(() => {
    const startIndex = (currentPage - 1) * ENTRIES_PER_PAGE;
    return recentVehicleEntries.slice(startIndex, startIndex + ENTRIES_PER_PAGE);
  }, [recentVehicleEntries, currentPage]);

  const totalPages = Math.ceil(recentVehicleEntries.length / ENTRIES_PER_PAGE);
